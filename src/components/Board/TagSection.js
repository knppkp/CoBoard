...
const TagSection = React.forwardRef(({ tags, btags, setTags }, ref) => {
  ...
  const availableTags = btags.filter(
    (btag) => !tags.some((tag) => tag.tag_id === btag.tag_id)
  );
  const filteredTags = availableTags
    .filter((btag) =>
      btag.tag_text.toLowerCase().includes(searchText.toLowerCase())
    )
    .slice(0, 5);
  ...
  return (
    <div ref={ref} className="p-2">
      <h1 className="text-gray1 font-bold text-2xl pl-5 -mb-3">Tags</h1>
      <div className="w-490 md:w-500 h-fit bg-basegreen mt-4 rounded-2xl py-4 px-10 flex flex-col items-center">
        ...
        <h2 className="text-xl self-start m-1 font-bold text-white">
          Add Tags
        </h2>
        <input
          type="text"
          ...
          className="bg-white w-350 md:w-430 h-10 m-1 p-3 rounded-2xl"
        />
        <div className="bg-white w-350 md:w-430 min-h-10 m-1 rounded-2xl flex flex-wrap p-2">
          ...
          {filteredTags.map((tag) => (
            <button
              key={tag.tag_id}
              className="m-1 px-3 py-2 rounded-md text-white bg-lightgreen"
